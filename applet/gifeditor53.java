...
public class gifeditor53 extends PApplet {
...
//  }
  ...
  for (int i = 0; i<fh; i++)
  {
    frameHolder[i].resize(PApplet.parseInt(imgWidth*.01f*sizeout),0);
...
//      textArea = "";
 ...
 // text(int(textValue), 360,195);
    ...
    println("here we go!!! " +PApplet.parseInt(theEvent.getGroup().getValue()));
    println("selection = "+inputFileNames[PApplet.parseInt(theEvent.getGroup().getValue())]);
 //   println("THE TEXT FIELD SAYS= "+int(controlP5.get(Textfield.class,"jump").getText()));
     movie = new Movie(this, inputFileNames[PApplet.parseInt(theEvent.getGroup().getValue())]);
      currentMovie=inputFileNames[PApplet.parseInt(theEvent.getGroup().getValue())];
     movie.loop();
     movie.pause();
     router=1;
    movie.jump(PApplet.parseFloat("jump"));
    maxImages = maxframes;
      background(0);
      imgCount=maxImages;
      ii=0;
...
public void movieEvent(Movie m) {
 ...
 //gifs[ii]=m.get(0, 0, m.width, m.height);
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--present", "--bgcolor=#666666", "--stop-color=#cccccc", "gifeditor53" });
  }
}
